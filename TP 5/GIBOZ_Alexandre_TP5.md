...
## Exercice 3 - Exchanger (suite)

1. **L'Exchanger est-il utilisable plus d'une fois ?**

Non, il n'est utilisable que pour un échange de valeurs.

2. **Dupliquer la classe Exchanger dans une classe ReusableExchanger et commencer par modifier votre méthode d'échange pour encoder l'état de l'Exchanger avec un enum.**

On créé la classe `ReusableExchanger`:
```java
